 ...
 * Features: Real Bot Avatar, Waving Hand Animation, Automatic Screen Floating
 */
(function() {
'use strict';
...
// ======== COMPANY CONFIG[span_5](start_span)[span_5](end_span) ========
const CONFIG = {
  ...
  const html = `
    ...
    <style>
      ...
      .waving-hand {
        ...
        bottom: -4px;
        font-size: 16px;
        animation: waveHand 1.2s infinite ease-in-out;
...
function initFloatingBot() {
  ...
  let vx = 1.0; // Horizontal drift speed
  let vy = 0.8; // Vertical drift speed
  let isHovered = false;
  ...
  function floatLoop() {
    // Float across screen only when chat is closed and not actively hovered
    if (!chatOpen && !isHovered) {
      ...
      // Screen boundary detection and smooth bouncing
      if (posX <= 10) {
        posX = 10;
        vx = -vx;
...
function getResponse(msg) {
  ...
  return `I can help you with: <br>• Security Systems<br>• Equestrian Products<br>• Free Quotes<br>• Contact Details<br><br>What would you like to know?`;
}
